...
function firstFieldErrorMessage(
  ...
  for (const v of Object.values(errors)) {
    ...
    if (typeof v === "object") {
      const nested = firstFieldErrorMessage(
        v as FieldErrors<ProductFormValues>,
      );
      if (nested) return nested;
    }
  }
  return "Confira nome, preço, NCM e dados fiscais.";
}
...
export const ProductWizardPage: React.FC = () => {
  ...
  const {
    ...
    resolver: zodResolver(ProductWizardSchema) as Resolver<ProductFormValues>,
    defaultValues: {
      name: "",
      sale_price: 0.01,
      active: true,
    ...
    (async () => {
      if (isCloudActive) {
        try {
          ...
          if (!cancelled && prod) reset(prod as ProductFormValues);
        } catch {
  ...
  const runTaxArchitect = async () => {
    ...
    setIsAnalyzing(true);
    const result = await geminiService.suggestSmartTaxBreakdown(
      watchedName,
      watchedPrice,
      watchedProfile,
    );
  ...
  const nextStep = async () => {
    if (currentStep === 1) {
      const fields = ["name", "sale_price", "category"] as const;
    ...
    if (currentStep === 2) {
      // Alinhar ao backend: NCM numérico com 8 dígitos (placeholder vazio quebrava o Zod)
      const raw = String(watch("tax_profile.ncm") ?? "").replace(/\D/g, "");
      const ncm =
        raw.length >= 8 ? raw.slice(0, 8) : raw.padStart(8, "0").slice(-8);
      setValue("tax_profile.ncm", ncm, {
        shouldValidate: true,
        shouldDirty: true,
      });
      const isValid = await trigger("tax_profile.ncm" as any);
      if (isValid) {
        setSubmitBlockedMessage(null);
        setCurrentStep(3);
      }
  ...
  const onSubmit = async (data: ProductFormValues) => {
    ...
    try {
      if (isCloudActive) {
        if (isEditMode) await api.updateProduct(id!, data);
  ...
  const onInvalid = (errs: FieldErrors<ProductFormValues>) => {
    setSubmitBlockedMessage(firstFieldErrorMessage(errs));
  };
        ...
        <div>
          <h2 className="text-2xl font-black text-slate-900 flex items-center gap-3 uppercase tracking-tighter">
            ...
            {isEditMode
              ...
              : "Engenharia de Produto & Lucro"}
          </h2>
          <p className="text-xs text-slate-500 font-medium uppercase tracking-widest">
            Otimize sua margem através da segregação fiscal inteligente.
          </p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-[48px] shadow-2xl border border-slate-100 overflow-hidden">
            ...
            <div className="space-y-12 animate-[fadeIn_0.3s_ease-out]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-4">
                  ...
                  <input
                    {...register("name")}
                    className="w-full p-6 bg-slate-50 border-none rounded-[24px] text-xl font-bold focus:ring-4 focus:ring-indigo-100 outline-none"
                    placeholder="Ex: Hamburguer Chicago"
                  />
                  {errors.name && (
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full pl-16 pr-6 py-6 bg-slate-50 border-none rounded-[24px] font-black text-3xl font-mono text-indigo-600 outline-none"
                    />
                  </div>
                  {errors.sale_price && (
                    <p className="text-xs font-bold text-red-500 ml-1">
                  ...
                  <div className="space-y-6 p-8 bg-slate-50 rounded-[40px] border border-slate-100">
                    ...
                    <div className="grid grid-cols-1 gap-3">
                      {Object.values(FiscalProfile).map((p) => (
                        <label
                          key={p}
                          className={`flex items-center justify-between p-4 rounded-2xl cursor-pointer border-2 transition-all ${watchedProfile === p ? "bg-indigo-600 border-indigo-700 text-white shadow-lg" : "bg-white border-slate-100 text-slate-400 hover:border-indigo-100"}`}
                        >
                          <div className="flex items-center gap-3">
                            <input
                              type="radio"
                              {...register("tax_profile.fiscal_profile")}
                              value={p}
                              className="hidden"
                            />
                            <div
                              className={`w-4 h-4 rounded-full border-2 ${watchedProfile === p ? "border-white bg-indigo-400" : "border-slate-200"}`}
                            ></div>
                            <span className="text-[10px] font-black uppercase tracking-widest">
                              {p}
                            </span>
                          </div>
                          {p === FiscalProfile.ARROJADO && (
                    ...
                    <p className="text-[9px] text-slate-400 font-medium leading-relaxed italic">
                      * O perfil arrojado decompõe o produto ao máximo para
                      aproveitar isenções de carnes e hortifruti.
                    </p>
                  </div>

                  <div className="space-y-4">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">
                      NCM Principal de Saída
                    </label>
                    <input
                      {...register("tax_profile.ncm")}
                      className="w-full p-6 bg-white border-2 border-slate-100 rounded-[24px] font-black text-xl font-mono tracking-widest outline-none focus:border-indigo-500"
                      placeholder="00000000"
                      inputMode="numeric"
                      maxLength={8}
                    />
                    {errors.tax_profile?.ncm && (
                      <p className="text-xs font-bold text-red-500 ml-1">
                        {errors.tax_profile.ncm.message}
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="lg:col-span-7">
                  {taxStrategy ? (
                              ...
                              >
                                ...
                                <div className="text-right">
                                  <p className="text-sm font-black font-mono">
                                    R$ {item.allocated_price.toFixed(2)}
                                  </p>
                                  <p
                                    className={`text-[9px] font-black uppercase ${item.is_tax_free ? "text-emerald-400" : "text-indigo-400"}`}
                                  >
                    ...
                    <div className="h-full bg-slate-50 rounded-[48px] border-4 border-dashed border-slate-200 flex flex-col items-center justify-center p-12 text-center space-y-4">
                      ...
                      <div>
                        <h4 className="font-black text-slate-400 uppercase tracking-widest">
                          IA Fiscal Desconectada
                        </h4>
                        <p className="text-xs text-slate-300 font-medium">
                          Preencha o nome e preço, e escolha um perfil para ver
                          a mágica da decomposição.
                        </p>
                      </div>
                    </div>
                  )}
            ...
            <div className="space-y-12 animate-[fadeIn_0.3s_ease-out]">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                ...
                <div className="bg-emerald-50 border-2 border-emerald-100 p-10 rounded-[48px] flex flex-col justify-center space-y-6">
                  <div className="flex items-center gap-4">
                    <div className="p-4 bg-emerald-600 text-white rounded-3xl">
                    ...
                    <div>
                      ...
                      <h4 className="text-2xl font-black text-emerald-900">
                        Redução de ~
                        {taxStrategy?.total_tax_free_percentage || 0}% de
                        Impostos
                      </h4>
                    </div>
                  </div>
                  <p className="text-xs text-emerald-600 font-medium leading-relaxed">
                    Ao utilizar a segregação por ingredientes, sua nota fiscal
                    será emitida com múltiplas linhas internas, garantindo que
                    você não pague imposto integral sobre o pão, carne e
                    vegetais.
                  </p>
                </div>
              </div>
        ...
        {submitBlockedMessage && (
          <div className="px-12 pb-4 -mt-4">
            <p className="text-sm font-bold text-red-600 bg-red-50 border border-red-100 rounded-2xl px-6 py-4">
              {submitBlockedMessage}
            </p>
          </div>
        )}
