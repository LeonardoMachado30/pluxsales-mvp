...
export type BarcodeSelectableItem = Product | Ingredient;

function isProduct(item: BarcodeSelectableItem): item is Product {
...
function formatSalePriceLine(item: BarcodeSelectableItem): string {
  if (isProduct(item)) {
    return `R$ ${item.sale_price.toFixed(2)}`;
  }
  return "R$ 0.00";
}
...
export const BarcodeGeneratorPage: React.FC = () => {
  ...
  const [selectedItems, setSelectedItems] = useState<BarcodeSelectableItem[]>(
    [],
  );
  ...
  useEffect(() => {
    const initial = location.state?.initialItem as
      | BarcodeSelectableItem
      | undefined;
    if (initial) {
  ...
  const handleToggleItem = (item: BarcodeSelectableItem) => {
  ...
  const filteredItems = useMemo(() => {
    const list: BarcodeSelectableItem[] = [
      ...allProducts,
      ...allIngredients,
    ];
    ...
    <div className="space-y-8 animate-[fadeIn_0.3s_ease-out] pb-32">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-slate-500 font-medium mt-1">
              Gerador de etiquetas de código de barras e identificação.
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 print:hidden">
        ...
        <div className="lg:col-span-1 space-y-6">
                ...
                return (
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div
                        className={`p-2 rounded-xl ${isSelected ? "bg-indigo-600 text-white" : "bg-slate-100 text-slate-400"}`}
                      >
          ...
          <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm space-y-6">
            <h3 className="text-lg font-black text-slate-900 uppercase tracking-tight flex items-center gap-2">
              <Settings2 className="w-5 h-5 text-indigo-600" /> Configurar
              Layout
            </h3>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h4 className="font-black uppercase tracking-tight">
                        Parecer de Rotulagem Aura
                      </h4>
                      <p className="text-[10px] text-indigo-200 font-bold uppercase tracking-widest">
                        Normas ANVISA & Metrologia
                      </p>
                    </div>
                  </div>
                  <button
                    onClick={() => setShowAuraConsult(false)}
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-8 w-full max-w-2xl overflow-y-auto custom-scrollbar p-4">
                {selectedItems.map((item, idx) => (
                  ...
                  >
                    ...
                    <p className="text-[8px] font-bold font-mono text-slate-500 uppercase">
                      {getLabelCode(item) || "789123456789"}
                    </p>
                    <div className="mt-2 text-[9px] font-black text-indigo-600">
                      {formatSalePriceLine(item)}
                    </div>
                  </div>
                ))}
              </div>
