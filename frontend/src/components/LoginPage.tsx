...
export const LoginPage: React.FC = () => {
  ...
  const onSubmit = async (data: LoginForm) => {
    ...
    setApiError(null);
    try {
      const user = await api.loginAndBuildUser(data.email, data.password);
      authService.login(user);
      navigate("/");
    } catch (e: unknown) {
      setApiError(e instanceof Error ? e.message : "Falha no login");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md relative z-10">
        <div className="text-center mb-10">
          ...
          <h1 className="text-4xl font-black text-white tracking-tighter mb-2">
            PluxSales <span className="text-indigo-500">SaaS</span>
          </h1>
          <p className="text-slate-400 font-medium">
            Acesse sua central de inteligência gastronômica.
          </p>
        </div>
        ...
        <div className="bg-white/5 backdrop-blur-xl p-10 rounded-[40px] border border-white/10 shadow-2xl">
          ...
          <div className="mt-8 pt-8 border-t border-white/5">
            <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest text-center mb-4">
