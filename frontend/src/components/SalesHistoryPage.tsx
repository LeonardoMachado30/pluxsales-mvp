...
export const SalesHistoryPage: React.FC = () => {
  ...
  const filteredSales = useMemo(() => {
    return sales.filter((sale) => {
      ...
      const matchesMethod =
        filterMethod === "ALL" || sale.payment_method === filterMethod;
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">
            Histórico completo de transações e conciliação financeira.
          </p>
        </div>
        <Link
          to="/sales"
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-slate-900 p-6 rounded-[32px] shadow-2xl shadow-slate-200 text-white relative overflow-hidden">
          ...
          <div className="text-4xl font-black font-mono tracking-tighter">
            <span className="text-lg font-bold mr-1 text-indigo-400">R$</span>
        ...
        {filteredSales.length > 0 ? (
                ...
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    <div className="flex items-center gap-3">
                      ...
                      <div
                        className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border ${getPaymentColor(sale.payment_method)}`}
                      >
                ...
                <div className="flex items-center gap-8 text-right border-t lg:border-t-0 pt-6 lg:pt-0 border-slate-50">
                  <div className="hidden sm:block">
                    ...
                    <div className="text-xs font-bold text-emerald-500 font-mono">
                      {(
                        ((sale.total_revenue - sale.total_cost) /
                          sale.total_revenue) *
                        100
                      ).toFixed(0)}
                      %
                  ...
                  <div>
                    <div className="text-[9px] font-black text-indigo-600 uppercase tracking-widest mb-1">
                      Valor Venda
                    </div>
              ...
              <div className="bg-slate-50/30 p-8 border-t border-slate-50">
                <div className="flex flex-wrap gap-4">
                  {sale.items.map((item, idx) => (
                    <div
                      key={idx}
                      className="flex items-center gap-4 bg-white px-4 py-3 rounded-2xl border border-slate-100 shadow-sm min-w-[200px]"
                    >
                      <div className="w-8 h-8 bg-slate-900 text-white rounded-xl flex items-center justify-center font-black text-xs">
                        {item.qty}
                      </div>
                      <div className="flex-1">
                        <div className="text-xs font-black text-slate-800 truncate max-w-[120px]">
                          {item.name}
                        </div>
                    ...
                    <div className="mt-6 flex items-center gap-6 p-4 bg-indigo-50/50 rounded-2xl border border-indigo-100/30 w-fit">
                      <div className="flex flex-col">
                        ...
                        <span className="text-sm font-black font-mono text-slate-800">
                          R$ {sale.received_amount.toFixed(2)}
                        </span>
                      </div>
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-sm font-black font-mono text-emerald-600">
                          R$ {sale.change_amount?.toFixed(2) || "0.00"}
                        </span>
                      </div>
                    </div>
                  )}
          ...
          <div className="py-32 text-center bg-white rounded-[48px] border-2 border-dashed border-slate-100">
            ...
            <p className="text-slate-500 text-sm max-w-xs mx-auto mt-2 font-medium">
              Não existem registros para os filtros selecionados ou ainda não
              houveram vendas.
            </p>
