...
export const KitchenDisplayPage: React.FC = () => {
  ...
  const getStatusColor = (status: TicketStatus) => {
    switch (status) {
      case TicketStatus.PENDING: return 'text-amber-500';
      case TicketStatus.PREPARING: return 'text-indigo-500';
      case TicketStatus.READY: return 'text-emerald-500';
    ...
    <div className="space-y-8 animate-[fadeIn_0.3s_ease-out] pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">Gerencie a fila de preparo em tempo real para otimizar o tempo de entrega.</p>
        </div>
        <div className="flex gap-2 bg-slate-100 p-1.5 rounded-[20px] border border-slate-200">
           {[TicketStatus.PENDING, TicketStatus.PREPARING, TicketStatus.READY].map(status => (
             <button
               key={status}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {filteredTickets.map(ticket => {
          const waitTime = Math.floor((new Date().getTime() - new Date(ticket.timestamp).getTime()) / 60000);
          const isUrgent = waitTime > 15;

          return (
            <div key={ticket.id} className={`bg-white rounded-[32px] border-2 transition-all flex flex-col shadow-sm ${isUrgent ? 'border-red-200 animate-pulse' : 'border-slate-100'}`}>
              {/* Header */}
              <div className={`p-6 rounded-t-[30px] flex justify-between items-start ${isUrgent ? 'bg-red-50' : 'bg-slate-50/50'}`}>
                <div>
                   <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Mesa / Setor</p>
                   <h3 className="text-lg font-black text-slate-800">{ticket.tableName || 'Balcão'} <span className="text-slate-400 font-medium">({ticket.sectorName})</span></h3>
                </div>
                <div className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-[9px] font-black uppercase border ${isUrgent ? 'bg-red-100 text-red-600 border-red-200' : 'bg-white text-slate-500'}`}>
                 ...
                 {ticket.items.map((item, idx) => (
                   <div key={idx} className="flex justify-between items-center group">
                      <div className="flex items-center gap-3">
                         ...
                         <span className="text-sm font-black text-slate-800 group-hover:text-indigo-600 transition-colors uppercase tracking-tight">{item.name}</span>
                      </div>
              ...
              <div className="p-6 border-t border-slate-50">
                 {ticket.status === TicketStatus.PENDING && (
                   <button 
                     onClick={() => handleUpdateStatus(ticket.id, TicketStatus.PREPARING)}
                 ...
                 {ticket.status === TicketStatus.PREPARING && (
                   <button 
                     onClick={() => handleUpdateStatus(ticket.id, TicketStatus.READY)}
                 ...
                 {ticket.status === TicketStatus.READY && (
                   <button 
                     onClick={() => handleUpdateStatus(ticket.id, TicketStatus.DELIVERED)}
        ...
        {filteredTickets.length === 0 && (
          <div className="col-span-full py-32 text-center bg-white rounded-[48px] border-4 border-dashed border-slate-100">
             ...
             <h3 className="text-xl font-black text-slate-800">Sem pedidos pendentes</h3>
             <p className="text-slate-500 text-sm max-w-xs mx-auto mt-2 font-medium uppercase tracking-widest">Aguardando novas transações do PDV.</p>
          </div>
        )}
      </div>
    </div>
