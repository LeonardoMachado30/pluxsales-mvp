...
import { Settings, Save, ArrowLeft, AlertTriangle, ShieldCheck, Cpu, CreditCard, User, Building, ExternalLink, Users, Plus, Trash2, Mail, ShieldAlert, Target } from 'lucide-react';
...
export const SettingsPage: React.FC = () => {
  ...
  const [settings, setSettings] = useState<AppSettings>({ lowStockThreshold: 100, monthlyGoal: 50000 });
    ...
    <div className="max-w-5xl mx-auto space-y-8 animate-[fadeIn_0.3s_ease-out] pb-24">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">Gestão de conta corporativa, segurança e equipe.</p>
        </div>
        <Link 
          to="/" 
        ...
        <div className="lg:col-span-1 space-y-4">
           ...
           <div className="bg-slate-900 p-8 rounded-[32px] text-white relative overflow-hidden group">
              <div className="relative z-10">
                 <p className="text-[9px] font-black uppercase tracking-widest text-indigo-400 mb-4">Seu Plano</p>
                 <h3 className="text-2xl font-black mb-1">{user?.status === 'PAID' ? 'Premium' : 'Período Trial'}</h3>
                 <p className="text-[10px] text-slate-500 font-bold uppercase mb-6">ID: {user?.tenantId}</p>
                 <button onClick={simulatePayment} className="w-full py-4 bg-white/10 hover:bg-white/20 border border-white/10 rounded-2xl font-black text-[10px] uppercase tracking-widest transition-all">
                    Ver Planos
                 </button>
              </div>
          ...
          {tab === 'PROFILE' && (
            <div className="bg-white rounded-[40px] border border-slate-100 shadow-sm overflow-hidden animate-[fadeIn_0.2s_ease-out]">
               <div className="p-10 space-y-12">
                  <section className="space-y-6">
                    <h3 className="text-lg font-black text-slate-900 uppercase tracking-tight flex items-center gap-3">
                       <Building className="w-5 h-5 text-indigo-600" /> Empresa & Tenant
                    </h3>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                       <div className="space-y-2">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">E-mail Corporativo</label>
                          <input disabled value={user?.email} className="w-full p-4 bg-slate-50 border-none rounded-2xl font-bold text-slate-800" />
                       </div>
                       <div className="space-y-2">
                          <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Tenant ID</label>
                          <div className="relative">
                             <input disabled value={user?.tenantId} className="w-full p-4 bg-slate-50 border-none rounded-2xl font-mono text-[10px] font-bold text-slate-400" />
                  ...
                  <section className="space-y-6">
                    <h3 className="text-lg font-black text-slate-900 uppercase tracking-tight flex items-center gap-3">
                       <AlertTriangle className="w-5 h-5 text-indigo-600" /> Regras de Inventário & Negócio
                    </h3>
                    <div className="space-y-4">
                       <div className="p-8 bg-slate-50 rounded-[32px] flex flex-col md:flex-row md:items-center justify-between gap-6">
                          <div>
                             <p className="text-sm font-black text-slate-800">Limiar Global de Estoque</p>
                             <p className="text-xs text-slate-500 font-medium">Define o ponto de alerta crítico em todo o sistema.</p>
                          </div>
                       ...
                       <div className="p-8 bg-slate-50 rounded-[32px] flex flex-col md:flex-row md:items-center justify-between gap-6">
                          <div className="flex items-center gap-4">
                             ...
                             <div>
                                <p className="text-sm font-black text-slate-800">Meta Mensal de Faturamento</p>
                                <p className="text-xs text-slate-500 font-medium">Alvo de faturamento para análise da Aura Forecaster.</p>
                             </div>
                          </div>
                          <div className="flex items-center gap-3">
                             <span className="text-sm font-black text-slate-400 font-mono">R$</span>
                             <input 
                               type="number"
               ...
               <div className="p-10 bg-slate-50/50 border-t border-slate-100 flex justify-end">
                  <button onClick={handleSave} className="px-12 py-4 bg-indigo-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-indigo-100 hover:scale-105 active:scale-95 transition-all">
            ...
            <div className="space-y-6 animate-[fadeIn_0.2s_ease-out]">
               <div className="bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm">
                  <div className="flex items-center justify-between mb-8">
                     <h3 className="text-lg font-black text-slate-900 uppercase tracking-tight flex items-center gap-3">
                        <Users className="w-5 h-5 text-indigo-600" /> Membros da Equipe
                     </h3>
                     <button 
                  ...
                  <div className="space-y-4">
                     {team.map(member => (
                        <div key={member.id} className="flex items-center justify-between p-5 border border-slate-100 rounded-2xl hover:bg-slate-50 transition-all">
                           <div className="flex items-center gap-4">
                              ...
                              <div>
                                 <p className="text-sm font-black text-slate-900 tracking-tight">{member.name} {member.id === user?.id && '(Você)'}</p>
                           ...
                           <div className="flex items-center gap-4">
                              <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border ${
                                ...
                                member.role === 'CASHIER' ? 'bg-blue-50 text-blue-700 border-blue-100' :
               ...
               <div className="p-8 bg-indigo-50 rounded-[40px] border border-indigo-100 flex items-center gap-4">
                  ...
                  <p className="text-xs font-bold text-indigo-900">
                    O cargo do usuário define o que ele pode ver e operar no PluxSales. <br/>
                    <span className="text-indigo-600 font-medium">Saiba mais sobre as hierarquias RBAC.</span>
                  </p>
               </div>
            </div>
          )}
            ...
            <div className="space-y-8 animate-[fadeIn_0.2s_ease-out]">
               <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  <div className="bg-white p-10 rounded-[40px] border-4 border-indigo-600 shadow-2xl relative overflow-hidden">
                     <div className="inline-block px-4 py-1.5 bg-indigo-600 text-white rounded-full text-[9px] font-black uppercase tracking-widest mb-6">Plano Atual</div>
                     <h4 className="text-3xl font-black text-slate-900 mb-2">Plux Premium</h4>
                     <p className="text-slate-500 text-sm font-medium mb-8">Toda a inteligência da Aura IA desbloqueada para seu negócio.</p>
                     
                     <div className="space-y-4 mb-10">
                        {['Audit Trails ilimitados', 'Aura Voice Assistance', 'Dashboard Financeiro', 'NCM Cloud Analysis'].map(f => (
                     ...
                     <div className="text-3xl font-black font-mono">R$ 149,90 <span className="text-sm font-bold text-slate-400">/mês</span></div>
                  </div>
                  ...
                  <div className="bg-slate-50 p-10 rounded-[40px] border border-slate-100 flex flex-col justify-center items-center text-center">
                     ...
                     <h4 className="text-xl font-black text-slate-800 mb-2">Histórico de Faturas</h4>
                     <p className="text-sm text-slate-500 font-medium mb-6">Visualize e baixe seus comprovantes de pagamento.</p>
                     <button className="text-[10px] font-black text-indigo-600 uppercase tracking-widest hover:underline">Solicitar Conciliação</button>
                  </div>
               </div>
            </div>
          )}
        ...
        <div className="fixed inset-0 z-[100] bg-slate-950/80 backdrop-blur-sm flex items-center justify-center p-6">
           <div className="bg-white rounded-[40px] w-full max-w-md overflow-hidden animate-[slideUp_0.3s_ease-out]">
              <div className="p-10 border-b border-slate-50">
                 ...
                 <p className="text-sm text-slate-500 font-medium">O convite será enviado para o e-mail do colaborador.</p>
              </div>
              <div className="p-10 space-y-6">
                 <div className="space-y-2">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Nome Completo</label>
                    <input className="w-full p-4 bg-slate-50 border-none rounded-2xl font-bold" placeholder="ex: Ricardo Silva" />
                 </div>
                 <div className="space-y-2">
                 ...
                 <div className="flex gap-4 pt-6">
                    <button onClick={() => setShowAddUser(false)} className="flex-1 py-4 font-black text-[10px] uppercase text-slate-400">Cancelar</button>
                    <button onClick={() => setShowAddUser(false)} className="flex-1 py-4 bg-indigo-600 text-white rounded-2xl font-black text-[10px] uppercase tracking-widest shadow-xl shadow-indigo-100">Enviar Convite</button>
                 </div>
              </div>
           </div>
        </div>
