...
const ProductCard: React.FC<{ product: Product; onDuplicate: (e: React.MouseEvent, p: Product) => void }> = ({ product, onDuplicate }) => {
  const navigate = useNavigate();
  const [isExpanded, setIsExpanded] = useState(false);
  const marginPercent = ((product.sale_price - product.cmv_total) / product.sale_price) * 100;
    ...
    <div className="bg-white rounded-[32px] shadow-sm border border-slate-100 hover:shadow-2xl hover:shadow-indigo-500/10 hover:border-indigo-100 transition-all duration-500 overflow-hidden flex flex-col">
      ...
      <div className="p-8">
        <div className="flex justify-between items-start mb-4">
          <div className="flex-1">
            ...
            <div className="mt-2 flex items-center gap-2">
               <div className={`flex items-center gap-1 text-[10px] font-black px-2.5 py-1 rounded-full border ${marginPercent > 60 ? 'bg-emerald-50 text-emerald-700 border-emerald-100' : 'bg-amber-50 text-amber-700 border-amber-100'}`}>
          ...
          <div className="text-right ml-4">
            <div className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-1">Preço Venda</div>
            <div className="font-black text-3xl text-slate-900 font-mono tracking-tighter">
              <span className="text-sm font-bold mr-0.5 text-indigo-600">R$</span>
        ...
        <div className="flex items-center gap-2 mt-6">
          ...
          <button
            onClick={() => navigate(`/labels`, { state: { initialItem: product } })}
      ...
      <div className={`overflow-hidden transition-all duration-500 ease-in-out ${isExpanded ? 'max-h-60 opacity-100' : 'max-h-0 opacity-0'}`}>
        <div className="px-8 pb-8 pt-2 border-t border-slate-50 space-y-4 bg-slate-50/30">
          ...
          <div className="flex justify-between items-center pt-2">
            <div className="space-y-1">
              ...
              <div className={`inline-flex items-center px-2 py-0.5 rounded-md text-[9px] font-black uppercase border ${product.active ? 'bg-emerald-50 border-emerald-200 text-emerald-700' : 'bg-slate-100 border-slate-200 text-slate-400'}`}>
  ...
  return (
    <div className="space-y-8 animate-[fadeIn_0.5s_ease-out]">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-white p-8 rounded-[40px] border border-slate-100 shadow-sm">
        <div>
          <h2 className="text-2xl font-black text-slate-900 tracking-tight">Catálogo de Ofertas</h2>
          <p className="text-slate-500 text-sm font-medium mt-1 italic">Gestão centralizada de preços e engenharia de margens.</p>
        </div>
        <Link
          to="/products/new"
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
        ...
        {products.length === 0 && (
          <div className="col-span-full py-32 text-center bg-white rounded-[48px] border-4 border-dashed border-slate-100">
             ...
             <h3 className="text-xl font-black text-slate-800">Cardápio sem itens</h3>
             <p className="text-slate-500 text-sm max-w-xs mx-auto mt-2 font-medium">Inicie a criação de fichas técnicas para popular sua vitrine de vendas e começar a lucrar.</p>
             <Link to="/products/new" className="inline-flex items-center gap-3 mt-10 px-10 py-5 bg-indigo-600 text-white rounded-[24px] font-black text-xs uppercase tracking-widest shadow-2xl shadow-indigo-100 hover:scale-105 transition-all">
