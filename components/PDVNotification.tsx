...
export const PDVNotification: React.FC = () => {
  ...
  return (
    <div className="fixed bottom-8 left-8 z-[200] space-y-4 max-w-sm w-full animate-[slideUp_0.3s_ease-out]">
      {notifications.map(n => (
        <div key={n.id} className="bg-slate-900 text-white p-5 rounded-[28px] shadow-2xl border border-white/10 flex items-start gap-4 group">
          ...
          <div className="flex-1">
             ...
             <p className="text-[11px] text-slate-400 font-medium leading-relaxed mt-1">{n.message}</p>
          </div>
          <button onClick={() => markRead(n.id)} className="p-1 text-slate-600 hover:text-white transition-colors">
             <X className="w-4 h-4" />
          </button>
        </div>
      ))}
