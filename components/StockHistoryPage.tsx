    ...
    <div className="space-y-8 animate-[fadeIn_0.3s_ease-out] pb-24">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">Histórico detalhado de entradas, saídas e ajustes manuais de estoque.</p>
        </div>
        <Link 
          to="/ingredients" 
            ...
            <table className="w-full text-left">
               <thead>
                  <tr className="bg-slate-50 border-b border-slate-100">
                     ...
                     <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Movimentação</th>
                     <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Quantidade</th>
                     <th className="px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest">Custo Momento</th>
                      ...
                      <tr key={log.id} className="hover:bg-slate-50/50 transition-colors">
                         <td className="px-8 py-6">
                            <div className="flex items-center gap-4">
                               ...
                               <span className="text-sm font-black text-slate-800">{ing?.name || 'Insumo Excluído'}</span>
                            </div>
                         </td>
                         <td className="px-8 py-6">
                            <div className={`inline-flex items-center gap-2 px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border ${
                               log.type === StockMovementType.IN ? 'bg-emerald-50 text-emerald-600 border-emerald-100' :
                               log.type === StockMovementType.OUT ? 'bg-amber-50 text-amber-600 border-amber-100' :
                               'bg-blue-50 text-blue-600 border-blue-100'
                            }`}>
                               {log.type === StockMovementType.IN ? <ArrowUpRight className="w-3 h-3" /> : <ArrowDownRight className="w-3 h-3" />}
                               {log.type}
                            </div>
                         </td>
                         <td className="px-8 py-6">
                            <span className="text-sm font-black font-mono">{log.qty.toLocaleString()} <span className="text-[10px] opacity-40">{ing?.unit_measure}</span></span>
                         </td>
                         <td className="px-8 py-6">
                            <span className="text-xs font-bold text-slate-500 font-mono">R$ {log.cost_at_moment.toFixed(4)}</span>
                         </td>
                         <td className="px-8 py-6">
                         ...
                         <td className="px-8 py-6">
                            <p className="text-xs text-slate-400 font-medium italic truncate max-w-[200px]">{log.notes || 'Sem observações'}</p>
                         </td>
                      </tr>
                    );
                  })}
